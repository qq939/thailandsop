...
public sealed class YoloSegmentationDetector : IDisposable
{
    ...
    private YoloSegmentationPostprocessor? _ortPostprocessor;

    public YoloSegmentationDetector(
        string modelPath,
    ...
    {
        _classNames = classNames;
        _ortPostprocessor?.UpdateClasses(classNames);
    }
    ...
    public YoloSegmentation[] Predict(Mat image)
    {
    ...
    private void CreateOrtComponents(InferenceDeviceKind deviceKind, OrtExecutionProviderKind? forcedProvider)
    {
        ...
        _ortPostprocessor = new YoloSegmentationPostprocessor(
    ...
    private OrtSessionFactoryOptions? BuildRuntimeOptions(InferenceDeviceKind deviceKind)
    {
        ...
        if (deviceKind == InferenceDeviceKind.GpuCuda)
        {
            ...
            if (TryResolveTensorRtCachePath(cacheKey, out var cachePath))
            {
                ...
                XTrace.WriteLine("[YoloSegDetection] TensorRT cache found at {0}, enabled.", cachePath);
            }
            else
            {
                options.ProviderOrder = new[]
                ...
                XTrace.WriteLine("[YoloSegDetection] TensorRT cache not found for key '{0}', falling back to CUDA.", cacheKey ?? "<null>");
            }
        }
    ...
    private static OrtSessionFactoryOptions? BuildSessionFactoryOptions(InferenceDeviceKind deviceKind, string? tensorRtCacheKey)
    {
